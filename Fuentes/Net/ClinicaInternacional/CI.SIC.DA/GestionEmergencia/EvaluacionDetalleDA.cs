        ...
        public void Grabar(EvaluacionDetalleBE evaluacionDetalle)
        {
            using (var cn = new SqlConnection(Configuraciones.CadenaConexion))
            {
                using (var cmd = new SqlCommand())
                {
                    ...
                    var parametrosSql = new[]
                    {
                        new SqlParameter("@evaluaciondetalle", SqlDbType.Int) { Value = evaluacionDetalle.IdEvaluacionDetalle, Direction = ParameterDirection.Input },
                        new SqlParameter("@verificacion", SqlDbType.Int) { Value = evaluacionDetalle.IdVerificacion, Direction = ParameterDirection.Input },
                        new SqlParameter("@conformidad", SqlDbType.Bit) { Value = evaluacionDetalle.Conformidad, Direction = ParameterDirection.Input },
                        new SqlParameter("@evaluacion", SqlDbType.Int) { Value = evaluacionDetalle.IdEvaluacion, Direction = ParameterDirection.Input }
                    };
                    cmd.Parameters.AddRange(parametrosSql);
                    cn.Open();
