...
,GM_Estado
,case GM_Estado when 1 then 'Registrado' when 2 then 'Atendido' else 'Anulado' end Estado
...
Select GM_NroOrden, GM_NroSolicitud, GM_Fecha_Creacion, GM_Estado
...
BEGIN
	...
	Select GM_NroOrden
		...
		,GM_Comentarios
		,GM_Estado
		,GM_CodigoSolicitante	NomTiposervicio
	FROM TB_ORDEN_TRABAJO
		where GM_Estado in (1,2,3)
			and (
				(@GM_Estado=0)
				or
				(@GM_Estado<>0 and GM_Estado = @GM_Estado)
				)
	ORDER BY GM_Estado,GM_Fecha_Creacion,GM_NroOrden
END
GO
